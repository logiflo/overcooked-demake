...
function createLevel1(game, collisionSystem, audioManager) {
  ...
  for (let cnt = 0; cnt < 7; cnt++) {
    ...
    if (cnt === 0 || cnt === 6) furniture = ["environment_02", "Table", null]
  ...
  for (let cnt = 1; cnt < 4; cnt++) {
    ...
    if (cnt === 3) furniture = ["environment_09", "Box", "fruits_1"];
  ...
  for (let cnt = 1; cnt < 4; cnt++) {
    let furniture = ["environment_03", "Table", null];
    if (cnt === 1) furniture = ["environment_08", "Box", "fruits_2"];
    ...
    if (cnt === 1) {
      entity.sprite.setName(furniture[2]);
      createEntity(game, (150 + 6 * entitySize.x), (100 + cnt * entitySize.y), ["fruits_2" , "Box"]);
    }
